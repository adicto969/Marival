...
if(isset($_SESSION['IDUser'])){
  $bdM = new ConexionM();
  $bdM->__constructM();
  $sql = $bdM->query("SELECT IDEmpresa, centro, DB, UserDB, PassDB, PC, TN, PP, PA, POR, correo, server, DoS, FactorA, supervisor, centros FROM config WHERE IDUser = '".$_SESSION['IDUser']."' LIMIT 1;");
  if($bdM->rows($sql) > 0){
